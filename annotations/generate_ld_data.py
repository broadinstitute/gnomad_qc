...
eur_w_ld_chr_ht = 'gs://konradk/ld_score/reference_data/eur_w_ld_chr/20.l2.ldscore.ht'
eur_ref_ld_chr_ht = 'gs://konradk/ld_score/reference_data/eur_ref_ld_chr/20.l2.ldscore.ht'
kg_ld_ht = 'gs://konradk/ld_score/reference_data/1000G_Phase3_baselineLD_v2.1_ldscores/baselineLD.20.l2.ldscore.ht'


def get_pop_and_subpop_counters(mt):
    cut_dict = {'pop': hl.agg.filter(hl.is_defined(mt.meta.pop), hl.agg.counter(mt.meta.pop)),
                'subpop': hl.agg.filter(hl.is_defined(mt.meta.subpop), hl.agg.counter(mt.meta.subpop))
                }
...
def import_reference_ld_scores():
    ht = hl.import_table(eur_w_ld_chr_ht.replace('.ht', '.gz'), impute=True, types={'CHR': hl.tstr})
    ht.key_by(locus=hl.locus(ht.CHR, ht.BP)).write(eur_w_ld_chr_ht)

    ht = hl.import_table(eur_ref_ld_chr_ht.replace('.ht', '.gz'), impute=True, types={'CHR': hl.tstr})
    ht.key_by(locus=hl.locus(ht.CHR, ht.BP)).write(eur_ref_ld_chr_ht)

    ht = hl.import_table(kg_ld_ht.replace('.ht', '.gz'), impute=True, types={'CHR': hl.tstr})
    ht = ht.select('CHR', 'BP', 'SNP', L2=ht.baseL2)
    ht.key_by(locus=hl.locus(ht.CHR, ht.BP)).write(kg_ld_ht)
...
def generate_ld_pruned_set(mt: hl.MatrixTable, pop_data: dict, data_type: str, r2: str = '0.2',
    ...
    for label, pops in dict(pop_data).items():
        for pop in pops:
            if pop in ('oth', 'onf', 'oea'): continue
