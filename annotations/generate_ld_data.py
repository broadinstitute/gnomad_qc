...
def generate_ld_scores_from_ld_matrix(pop_data, data_type, min_frequency=0.01, call_rate_cutoff=0.8,
    ...
    for label, pops in dict(pop_data).items():
        for pop, n in pops.items():
            ...
            out_name = ld_scores_path(data_type, pop, adj)
...
def generate_cross_pop_ld_scores_from_ld_matrices(pop1, pop2, data_type, pop_data, min_frequency=0.01, call_rate_cutoff=0.8,
    ...
    ht2 = hl.read_table(ld_index_path(data_type, pop2, adj=adj))
    ...
    r1 = BlockMatrix.read(ld_matrix_path(data_type, pop1, min_frequency >= COMMON_FREQ, adj=adj)).filter(indices1, indices1)
    r2 = BlockMatrix.read(ld_matrix_path(data_type, pop2, min_frequency >= COMMON_FREQ, adj=adj)).filter(indices2, indices2)
    r_bm = r1 * r2
