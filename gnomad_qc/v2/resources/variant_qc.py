...
def get_2_0_2_rf_path(data_type: str, beta: bool):
    ...
    * Exomes beta only contain release sites
    ...
    if data_type == "exomes":
        return "gs://gnomad/annotations/hail-0.2/ht/exomes/score_rankings/{}".format(
            "gnomad_exomes.rf.170620_new.txt.bgz"
            if beta
            else "gnomad.exomes.variantqc.txt.bgz"
        )
    else:
        return "gs://gnomad/annotations/hail-0.2/ht/genomes/score_rankings/{}".format(
            "rf_as_qd_max_pab.rf.txt.bgz"
            if beta
            else "gnomad.sites.RF.newStats24.txt.bgz"
        )
...
def rf_run_hash_path(data_type: str, hail_version: str = CURRENT_HAIL_VERSION):
    ...
    return "gs://gnomad/annotations/hail-{0}/ht/{1}/rf/gnomad.{1}.runs.json".format(
        hail_version, data_type
    )


def rf_annotated_path(
    data_type: str, adj: bool = False, hail_version: str = CURRENT_HAIL_VERSION
) -> str:
    """
    ...
    return "gs://gnomad/annotations/hail-{0}/ht/{1}/rf/gnomad.{1}.{2}.ht".format(
        hail_version, data_type, "adj" if adj else "raw"
    )
...
def rf_path(
    ...
    return (
        "gs://gnomad/annotations/hail-{0}/ht/{1}/rf/{2}/gnomad.{1}.{2}.{3}.{4}".format(
            hail_version, data_type, run_hash, data, extension
        )
    )
...
def release_vcf_path(
    data_type: str, release_tag=RELEASE_VERSION, contig=None, coding_only=False
):
    """
...
def get_ucsc_mappability():
    ucsc_mappability = hl.import_table(
        "gs://gcp-public-data--gnomad/resources/ucsc/wgEncodeDukeMapabilityUniqueness35bp.bedGraph",
        impute=True,
        no_header=True,
    )
