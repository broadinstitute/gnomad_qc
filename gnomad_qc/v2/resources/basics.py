...
def get_release_file(file_path: str, version: str = CURRENT_RELEASE) -> str:
    ...
    if file_exists(file_path.format(version)):
        ...
        print(
            "ERROR: Resource {} could not be found for any release.".format(
                file_path.format(version)
            )
        )
        return file_path.format(version)
...
def public_exomes_ht_path(split=True, version=CURRENT_RELEASE):
    if int(version[0]) > 1 and int(version[2]) > 0:
        ...
        raise FileNotFoundError(
            f"Public release files are no longer available for versions under 2.1."
        )
...
def get_gnomad_public_data(data_type, split=True, version=CURRENT_RELEASE):
    ...
    return hl.read_table(
        get_gnomad_public_data_path(data_type, split=split, version=version)
    )
...
def get_gnomad_data(
    ...
    if non_refs_only:
        ...
        mt = hl.read_matrix_table(
            get_gnomad_data_path(
                data_type, hardcalls=not raw, split=split, hail_version=hail_version
            )
        )
...
def get_gnomad_meta(
    data_type: str, version: str = None, full_meta: bool = False
) -> hl.Table:
    """
    ...
    """
    if data_type == "joint":
        exomes_ht = hl.read_table(get_gnomad_meta_path("exomes", version)).key_by(
            "s", data_type="exomes"
        )
        exomes_ht = exomes_ht.annotate(qc_platform=hl.str(exomes_ht.qc_platform))
        genomes_ht = hl.read_table(get_gnomad_meta_path("genomes", version)).key_by(
            "s", data_type="genomes"
        )
...
def get_gnomad_data_path(
    data_type,
    hardcalls=False,
    split=True,
    non_refs_only=False,
    hail_version=CURRENT_HAIL_VERSION,
):
    """
    Wrapper function to get paths to gnomAD data
    ...
    if hardcalls:
        ...
        return (
            raw_exomes_mt_path(hail_version)
            if data_type == "exomes"
            else raw_genomes_mt_path(hail_version)
        )
...
def sample_annotations_table_path(
    data_type, annotation_type, hail_version=CURRENT_HAIL_VERSION
):
    """
...
def gnomad_public_pca_loadings_ht_path(
    version: str = CURRENT_RELEASE, subpop: str = ""
):
    """
...
def fam_path(
    data_type: str, version: str = CURRENT_FAM, true_trios: bool = False
) -> str:
    """
...
def kgp_phase3_genotypes_mt_path(
    split: bool = True, hail_version=CURRENT_HAIL_VERSION
) -> str:
    """
...
def methylation_sites_ht_path(ref: str = "GRCh37"):
    if ref not in REFERENCE_DATA:
        raise DataException(
            "Select reference as one of: {}".format(",".join(REFERENCES))
        )
