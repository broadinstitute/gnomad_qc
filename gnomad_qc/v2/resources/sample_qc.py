...
def get_liftover_v2_qc_mt(
    data_type: str, ld_pruned: bool, release_only: bool = False, overwrite: bool = False
) -> hl.MatrixTable:
    """
    ...
    if not overwrite and hl.hadoop_exists(path):
        ...
        grch38_qc_mt = grch38_qc_mt.transmute_rows(
            locus=hl.liftover(grch38_qc_mt.locus, "GRCh38"), locus37=grch38_qc_mt.locus
        )
        grch38_qc_mt = grch38_qc_mt.filter_rows(
            grch38_qc_mt.locus.contig == "chr" + grch38_qc_mt.locus37.contig
        )
        grch38_qc_mt = grch38_qc_mt.key_rows_by(
            locus=grch38_qc_mt.locus, alleles=grch38_qc_mt.alleles
        )
...
def qc_mt_path(
    data_type: str, ld_pruned: bool = False, reference_genome: str = "GRCh37"
) -> str:
    """
...
def qc_ht_path(data_type: str, part: str) -> str:
    ...
    if part not in ("hard_filters", "platforms", "pop_platform"):
        raise DataException(
            "Select part as one of 'hard_filters', 'platforms', or 'pop_platform'"
        )
...
exome_callrate_scores_ht_path = (
    "gs://gnomad/sample_qc/ht/gnomad.exomes.callrate_pca_scores.ht"
)
...
estonian_batches = (
    "gs://gnomad/sample_qc/input_meta/gnomad.genomes.estonian_samples.txt"
)
