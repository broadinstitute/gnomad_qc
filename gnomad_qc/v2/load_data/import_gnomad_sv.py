...
def merge_genomes_sv_samples(
    genomes_mt: hl.MatrixTable, sv_mt: hl.MatrixTable
) -> hl.Table:
    ...
    gnomad_sv_samples = gnomad_sv_samples.annotate(in_v2_sv=True)
    ...
    merged_samples.group_by("in_v2_genomes", "in_v2_sv").aggregate(
        n=hl.agg.count()
    ).show()
...
def order_cols(
    mt1: hl.MatrixTable, mt2: hl.MatrixTable
    ...
    return (mt1.choose_cols(cols.idx1.collect()), mt2.choose_cols(cols.idx2.collect()))


def merge_with_short_variants(min_af: float):
    # Load gnomAD genomes and filter to PASS / release sites and samples
    gnomad_genomes = get_gnomad_data(
        "genomes", adj=True, release_samples=True, release_annotations=True
    )
    gnomad_genomes = gnomad_genomes.filter_rows(
        (hl.len(gnomad_genomes.filters) == 0) & (gnomad_genomes.popmax[0].AF >= min_af)
    )
    ...
    gnomad_sv = gnomad_sv.filter_rows(hl.len(gnomad_sv.filters) == 0)
    ...
    gnomad_sv = gnomad_sv.annotate_cols(genomes_s=merged_samples[gnomad_sv.s].genomes_s)
    ...
    gnomad_genomes = gnomad_genomes.key_cols_by(s=merged_samples[gnomad_genomes.s].sv_s)

    # Unify global, col, row and entry schemas
    gnomad_genomes = gnomad_genomes.select_entries("GT", GQ=hl.null(hl.tint32))
    gnomad_genomes = gnomad_genomes.select_rows(sv_info=hl.null(gnomad_sv.info.dtype))
    ...
    merged_mt = merged_mt.annotate_globals(min_af=min_af)
    return merged_mt
...
def generate_hists():
    ...
    logger.info(
        "Found {} samples with age data.".format(
...
def main(args):
    ...
    if args.generate_hists:
        ...
        hists_ht.write(
            "gs://gnomad-public-requester-pays/papers/2019-sv/gnomad_sv_hists.ht",
            overwrite=args.overwrite,
        )
