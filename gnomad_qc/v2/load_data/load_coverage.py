...
def main(args):
        ...
        for base, anchored, sample_data in all_file_data:
            ...
            if anchored:
                mt = hl.import_matrix_table(
                    fname,
                    no_header=True,
                    row_fields={"f0": hl.tstr, "f1": hl.tint},
                ...
                mt = hl.import_matrix_table(
                    fname, no_header=True, min_partitions=num_partitions, force_bgz=True
                )
            mt = mt.key_cols_by(s=hl.literal(sample_data)[mt.col_id]).drop("col_id")
            mt.transmute_entries(coverage=mt.x).write(
                f"{root}/parts/part_{base}.mt", args.overwrite
            )
    ...
    if args.merge_coverage_mts:
        ...
        for i in range(chunks):
            ...
            for j in range(1, chunks):
                if i * chunks + j >= len(all_file_data):
                    break
                base, anchored, sample_data = all_file_data[i * chunks + j]
                next_mt = hl.read_matrix_table(
                    f"{root}/parts/part_{base}.mt"
                ).select_rows()
        ...
        for i in range(1, chunks):
            try:
                next_mt = hl.read_matrix_table(
                    f"{root}/intermediates/intermediate_{i}.mt"
                )
                mt = mt.union_cols(next_mt)
            except Exception:
                pass
    ...
    if args.aggregate_coverage:
        ...
        ht = ht.transmute(
            **{
                f"over_{x}": hl.sum(mt.count_array[x:])
        ...
        if args.exomes:
            ...
            ht = ht.checkpoint(
                "gs://gnomad-tmp/coverage/genomes_agg.ht", overwrite=args.overwrite
            )
    ...
    if args.aggregate_coverage_platforms:
        ...
        if data_type == "genomes":
            meta_ht = meta_ht.annotate(
                qc_platform=hl.cond(meta_ht.pcr_free, "pcr_free", "pcr+")
            )

        mt = hl.read_matrix_table(coverage_mt_path(data_type))
        mt = mt.annotate_cols(
            meta=meta_ht.select("release", "qc_platform", "sex")[mt.s]
        )
        mt = mt.filter_cols(mt.meta.release)

        n = mt.aggregate_cols(
            hl.agg.counter((mt.meta.qc_platform, mt.meta.sex)), _localize=False
        )

        grp_mt = mt.group_cols_by(mt.meta.qc_platform, mt.meta.sex).aggregate(
            mean=hl.agg.mean(mt.coverage),
            median=hl.agg.approx_quantiles(mt.coverage, 0.5),
            count_array=hl.rbind(
                hl.agg.counter(hl.min(100, mt.coverage)),
        ...
        grp_mt = grp_mt.transmute_entries(
            **{
                f"over_{x}": hl.sum(grp_mt.count_array[x:])
                for x in [1, 5, 10, 15, 20, 25, 30, 50, 100]
            }
        )

        grp_mt = grp_mt.annotate_cols(n=n[(grp_mt.qc_platform, grp_mt.sex)])
        if args.exomes:
            grp_mt.write(coverage_mt_path(data_type, grouped=True), args.overwrite)
        else:
            grp_mt.checkpoint(
                "gs://gnomad-tmp/coverage/genomes_agg_grouped.mt",
                overwrite=args.overwrite,
            )
            grp_mt.key_rows_by("locus").write(
                coverage_mt_path(data_type, grouped=True), args.overwrite
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--read_coverage_files", help="Read raw coverage .gz files", action="store_true"
    )
    parser.add_argument(
        "--merge_coverage_mts",
        help="Merge individual coverage .mt files",
        action="store_true",
    )
    parser.add_argument(
        "--aggregate_coverage", help="Aggregate coverage data", action="store_true"
    )
    parser.add_argument(
        "--aggregate_coverage_platforms",
        help="Aggregate coverage data by platform and sex",
        action="store_true",
    )
    parser.add_argument(
        "--export_coverage", help="Export coverage data", action="store_true"
    )
    parser.add_argument(
