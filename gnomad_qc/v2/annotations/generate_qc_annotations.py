...
from gnomad.utils.annotations import add_variant_type, unphase_call_expr
...
def generate_qc_annotations(
    mt: hl.MatrixTable, all_annotations: bool = True, medians: bool = True
) -> hl.Table:
    """
    Writes bi-allelic sites MT with the following annotations:
    ...
    for group in sample_group_filters.keys():
        ...
        qual_agg = -10 * hl.agg.filter(
            non_refs, hl.agg.sum(hl.cond(mt.PL[0] > 3000, -300, hl.log10(gp0)))
        )
        ...
        if all_annotations:
            stats_expression = stats_expression.annotate(
                ...
                combined_pab=hl.or_missing(
                    ...
                    * hl.log10(
                        hl.pchisqtail(
                            -2 * hl.agg.filter(hets, hl.agg.sum(hl.log10(pab))),
                            2 * het_length,
                        )
                    ),
                ),
...
def generate_qual_hists(mt: hl.MatrixTable) -> hl.Table:
    ...
    return mt.annotate_rows(
        ...
        ab_hist_alt=hl.agg.filter(
            mt.GT.is_het(), hl.agg.hist(mt.AD[1] / hl.sum(mt.AD), 0, 1, 20)
        ),
    ).rows()


def generate_family_stats(
    mt: hl.MatrixTable, fam_file: str, calculate_adj: bool = False
...
def family_stats(
    mt: hl.MatrixTable, ped: hl.Pedigree, group_name: str
) -> Tuple[hl.expr.StructExpression, hl.Table]:
    ...
    family_stats_struct = hl.struct(
        ...
        unrelated_qc_callstats=hl.agg.filter(
            mt.high_quality & mt.unrelated_sample, hl.agg.call_stats(mt.GT, mt.alleles)
        ),
...
def read_fam(fam_file: str) -> hl.Table:
    ...
    return (
        hl.import_table(fam_file, no_header=True)
...
def generate_de_novos(
    mt: hl.MatrixTable, fam_file: str, freq_data: hl.Table
) -> hl.Table:
    mt = mt.select_cols()
    fam_ht = read_fam(fam_file).key_by()
    fam_ht = (
        fam_ht.select(s=[fam_ht.s, fam_ht.pat_id, fam_ht.mat_id])
        .explode("s")
        .key_by("s")
    )
    ...
    de_novo_table = de_novo_table.key_by("locus", "alleles").collect_by_key(
        "de_novo_data"
    )
...
def annotate_truth_data(mt: hl.MatrixTable) -> hl.Table:
    ...
    truth_mtes = {
        key: hl.split_multi_hts(
            hl.read_matrix_table(path).repartition(1000), left_aligned=False
        )
        for key, path in truth_mtes.items()
    }

    return mt.annotate_rows(
        truth_data=hl.struct(
            **{
                root: hl.is_defined(truth_mt[mt.row_key, :])
                for root, truth_mt in truth_mtes.items()
            }
        )
    ).rows()
...
def main(args):
    ...
    if args.vep:  # CPU-hours: 250 (E), 600 (G)
        ...
        hl.vep(mt, vep_config).write(
            annotations_ht_path(data_type, "vep"), args.overwrite
        )

        mt = get_gnomad_data(data_type).rows().select()
        hl.vep(mt, vep_config, csq=True).write(
            annotations_ht_path(data_type, "vep_csq"), args.overwrite
        )

    if args.generate_allele_data:  # CPU-hours: 100 (E), 200 (G)
        mt = get_gnomad_data(data_type, split=False)
        generate_allele_data(mt).write(
            annotations_ht_path(data_type, "allele_data"), args.overwrite
        )

    if (
        args.generate_qc_annotations
    ):  # CPU-hours: 750 (E), 2400 (G) - ~1 individual task each runs long
    ...
    if args.generate_call_stats:  # CPU-hours: 750 (E), 1500 (G)
        ...
        generate_call_stats(mt).write(
            annotations_ht_path(data_type, "call_stats"), args.overwrite
        )

    if args.generate_family_stats:  # CPU-hours: 8K (E), 13K (G)
        mt = get_gnomad_data(data_type)
        mt, sample_table = generate_family_stats(
            mt, fam_path(data_type, true_trios=True), args.include_adj_family_stats
        )
        write_temp_gcs(
            mt, annotations_ht_path(data_type, "family_stats"), args.overwrite
        )
        write_temp_gcs(
            sample_table,
            sample_annotations_table_path(data_type, "family_stats"),
            args.overwrite,
        )

    if (
        args.generate_de_novos
    ):  # (2.2 min/part @ 100K = 3K CPU-hours) + (7.4 m/p = 12K) + (34 m/p = ~44K) = 59K
        # Turn on spark speculation?
    ...
    if args.annotate_truth_data:
        ...
        annotate_truth_data(mt).write(
            annotations_ht_path(data_type, "truth_data"), args.overwrite
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--generate_qual_hists",
        help="Calculates GQ, DP, AB histograms per variant",
        action="store_true",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--generate_de_novos", help="Calculates de novo data", action="store_true"
    )
    parser.add_argument(
        "--annotate_truth_data",
        help="Annotates MT with truth data",
        action="store_true",
    )
    parser.add_argument(
        "--calculate_medians",
