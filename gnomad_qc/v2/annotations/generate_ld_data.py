...
def get_pop_and_subpop_counters(mt):
    cut_dict = {
        ...
        "subpop": hl.agg.filter(
            hl.is_defined(mt.meta.subpop)
            & (mt.meta.subpop != "oea")
            & (mt.meta.subpop != "onf"),
            hl.agg.counter(mt.meta.subpop),
        ),
    }
...
def filter_mt_for_ld(
    mt, label, pop, common_only: bool = True, sv_dataset: bool = False
):
    ...
    meta_index = (
        hl.zip_with_index(pop_mt.freq_meta)
...
def export_snv_sv_ld_matrix(
    ...
    for label, pops in dict(pop_data).items():
        for pop in pops:
            if pop not in SNV_SV_POPS:
                continue
            bm = BlockMatrix.read(
                ld_resources._ld_matrix_path(data_type, pop, common_only, adj)
            )
            ld_index = hl.read_table(
                ld_resources._ld_index_path(data_type, pop, common_only, adj)
            )
            ...
            hl.read_table(ld_resources._ld_snv_sv_path(pop)).export(
                ld_resources._ld_snv_sv_path(pop).replace(".ht", ".txt.bgz")
            )
            snvs = snvs.add_index().key_by()
            svs = svs.add_index().key_by()
            snvs.select(
                chrom=snvs.locus.contig,
                pos=snvs.locus.position,
                ref=snvs.alleles[0],
                alt=snvs.alleles[1],
                i=snvs.idx,
            ).export(ld_resources._ld_snv_sv_index_path(pop, "snv"))
            svs.select(
                chrom=svs.locus.contig,
                pos=svs.locus.position,
                ref=svs.alleles[0],
                alt=svs.alleles[1],
                j=svs.idx,
            ).export(ld_resources._ld_snv_sv_index_path(pop, "sv"))
...
def generate_all_cross_pop_ld_scores(
    pop_data,
    data_type,
    min_frequency=0.01,
    call_rate_cutoff=0.8,
            ...
            for label, pops in dict(pop_data).items():
                for pop2, n in pops.items():
                    if (pop1, pop2) in (
                        ...
                        generate_cross_pop_ld_scores_from_ld_matrices(
                            pop1,
                            pop2,
                            data_type,
                            pop_data,
...
def generate_cross_pop_ld_scores_from_ld_matrices(
    ...
    ht2 = ht2.filter(
        (ht2.pop_freq.AF >= min_frequency)
        & (ht2.pop_freq.AF <= 1 - min_frequency)
        & (ht2.pop_freq.AN / n2 >= 2 * call_rate_cutoff)
    )

    ht1 = (
        ht1.filter(hl.is_defined(ht2[ht1.key]))
        .add_index(name="new_idx")
        .checkpoint(
            f"{temp_bucket}/{pop1}_{pop2}.ht",
            overwrite=overwrite,
            _read_if_exists=not overwrite,
        )
    )
    ht2 = (
        ht2.filter(hl.is_defined(ht1[ht2.key]))
        .add_index(name="new_idx")
        .checkpoint(
            f"{temp_bucket}/{pop2}_{pop1}.ht",
            overwrite=overwrite,
            _read_if_exists=not overwrite,
        )
    )
    ...
    r1 = BlockMatrix.read(
        ld_resources._ld_matrix_path(
            data_type, pop1, min_frequency >= COMMON_FREQ, adj=adj
        )
    ).filter(indices1, indices1)
    r2 = BlockMatrix.read(
        ld_resources._ld_matrix_path(
            data_type, pop2, min_frequency >= COMMON_FREQ, adj=adj
        )
    ).filter(indices2, indices2)
    r_bm = r1 * r2
...
def main(args):
    ...
    mt = get_gnomad_data(
        data_type, release_samples=True, release_annotations=True, adj=args.adj
    )
    pop_data = get_pop_and_subpop_counters(mt)

    if args.snv_sv_comparison:
        mt = hl.read_matrix_table(
            "gs://gnomad/projects/genomes_sv_ld/genomes_union_sv.mt"
        )
        mt = mt.annotate_cols(meta=get_gnomad_meta("genomes")[mt.genomes_s])
        if args.adj:
            mt = mt.filter_entries(hl.is_missing(mt.GQ) | (mt.GQ >= 20))
        mt = mt.annotate_rows(
            **get_gnomad_public_data("genomes")[mt.row_key]
        ).annotate_globals(**get_gnomad_public_data("genomes").index_globals())
        pop_data = get_pop_and_subpop_counters(mt)
        generate_ld_matrix(
            mt,
            pop_data,
        ...
        export_snv_sv_ld_matrix(
            pop_data, "genomes_snv_sv", args.common_only, args.adj, args.overwrite
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--snv_sv_comparison",
        help="Compute LD between SNVs and SVs.",
        action="store_true",
    )
    parser.add_argument(
        "--generate_ld_pruned_set",
    ...
    parser.add_argument(
        "--cross_pop_ld_scores",
        help="Calculates cross-pop LD scores from LD matrix",
        action="store_true",
    )
    parser.add_argument(
        "--min_frequency",
