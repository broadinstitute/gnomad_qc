...
def add_faf_expr(
    ...
    freqs_to_use = hl.filter(
        lambda f: ((f.meta.size() == 1) & (f.meta.get("group") == "adj"))
        | (
            (f.meta.size() == 2)
            & (f.meta.get("group") == "adj")
            & pops_to_use.contains(f.meta.get("pop"))
        )
        | (
...
def generate_frequency_data(
    ...
    cut_dict = {
        "pop": hl.agg.filter(hl.is_defined(mt.meta.pop), hl.agg.counter(mt.meta.pop)),
        "sex": hl.agg.filter(
            hl.is_defined(mt.meta.sex), hl.agg.collect_as_set(mt.meta.sex)
        ),
    ...
    if calculate_downsampling:
        ...
        sample_group_filters.extend(
            [
                ...
                for ds in downsamplings
                for pop, pop_count in cut_data.pop.items()
                if ds <= pop_count
            ]
        )
    mt = mt.select_cols(
        group_membership=tuple(x[1] for x in sample_group_filters),
    ...
    mt = mt.annotate_rows(
        ...
        age_hist_het=hl.agg.filter(
            mt.adj & mt.GT.is_het(), hl.agg.hist(mt.age, 30, 80, 10)
        ),
        age_hist_hom=hl.agg.filter(
            mt.adj & mt.GT.is_hom_var(), hl.agg.hist(mt.age, 30, 80, 10)
        ),
    )
    if calculate_downsampling:
...
def generate_consanguineous_frequency_data(
    ht: hl.Table, f_ht: hl.Table, mt: hl.MatrixTable
) -> hl.Table:
    mt = mt.annotate_cols(meta=mt.meta.annotate(f=f_ht[mt.col_key].pop_f))
    projects_to_use = hl.literal(
        {
            "PROMIS",
            "T2D_28K_PROMIS_exomes",
            "T2D_28K_Singapore_S.Asian_exomes",
            "T2DGENES",
        }
    )
    consang_call_stats = hl.agg.filter(
        (mt.meta.f.f_stat >= F_CUTOFF)
        & mt.adj
        & (mt.meta.pop == "sas")
        & projects_to_use.contains(mt.meta.project_description),
        hl.agg.call_stats(mt.GT, mt.alleles),
    )
    consang_call_stats_bind = hl.bind(
        lambda cs: cs.annotate(
            AC=cs.AC[1], AF=cs.AF[1], homozygote_count=cs.homozygote_count[1]
        ),
        consang_call_stats,
    ...
    ht = ht.annotate_globals(
        freq_meta=ht.freq_meta.append(
            {"group": "adj", "pop": "sas", "sample_set": "consanguineous"}
        )
    )
    return ht.annotate(freq=ht.freq.append(mt.rows()[ht.key].consang))
...
def main(args):
    ...
    location = (
        f"frequencies_{args.subset_samples_by_field}"
        if args.subset_samples_by_field
        else "frequencies"
    )
    if args.calculate_frequencies:
        ht, sample_table = generate_frequency_data(
            mt, args.downsampling, args.by_platform
        )

        write_temp_gcs(ht, annotations_ht_path(data_type, location), args.overwrite)
        if args.downsampling:
            sample_table.write(
                sample_annotations_table_path(data_type, "downsampling"), args.overwrite
            )
    ...
    if args.add_consanguineous_frequencies and not args.genomes:
        ...
        write_temp_gcs(
            ht,
            annotations_ht_path(data_type, f"{location}_with_consanguineous"),
            args.overwrite,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--downsampling",
        help="Also calculate downsampling frequency data",
        action="store_true",
    )
    parser.add_argument(
        "--calculate_frequencies",
        help="Calcualte most frequency data",
        action="store_true",
    )
    parser.add_argument(
        "--add_consanguineous_frequencies",
        help="Add consanguineous frequencies to existing file",
        action="store_true",
    )
    parser.add_argument(
        "--by_platform",
