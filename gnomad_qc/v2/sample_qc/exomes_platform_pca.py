...
from gnomad_qc.v2.resources import evaluation_intervals_path, get_gnomad_data
...
def assign_platform_pcs(platform_pc_table: hl.Table) -> hl.Table:
    ...
    logger.info(
        "Assigning platforms to {} exome samples in MT...".format(len(callrate_data))
    )
...
def main(args):
    ...
    if not args.skip_prepare_data_for_platform_pca:
        ...
        mt = get_gnomad_data(
            "exomes", adj=True, raw=False, meta_root=None, fam_root=None, split=False
        )
    ...
    if not args.skip_run_platform_pca:
        ...
        callrate_mt = callrate_mt.annotate_entries(
            callrate=hl.int(callrate_mt.callrate > 0.25)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--skip_prepare_data_for_platform_pca",
        help="Skip prepping data for platform imputation (assuming already done)",
        action="store_true",
    )
    parser.add_argument(
        "--skip_run_platform_pca",
        help="Skip platform PCA (assuming already done)",
        action="store_true",
    )
    parser.add_argument(
