...
from gnomad_qc.v2.resources import (
    ...
    metadata_exomes_tsv_path,
    metadata_genomes_ht_path,
    metadata_genomes_tsv_path,
)
...
def add_release_annotations(ht: hl.Table) -> hl.Table:
    ...
    """
    ht = ht.annotate(
        high_quality=(hl.len(ht.hard_filters) == 0)
...
def collapse_small_pops(ht: hl.Table, min_pop_size: int) -> hl.Table:
    ...
    pop_collapse = (
        hl.literal(pop_collapse) if pop_collapse else hl.empty_dict(hl.tstr, hl.tstr)
    )

    subpop_counts = ht.aggregate(hl.agg.filter(ht.release, hl.agg.counter(ht.subpop)))
    subpop_collapse = {
        subpop: get_subpop_oth(subpop)
        for subpop, n in subpop_counts.items()
        if n < min_pop_size
    }
    subpop_collapse = (
        hl.literal(subpop_collapse)
        if subpop_collapse
...
def add_topmed_annotation(ht: hl.Table, data_type: str, cutoff: float):
    ...
    topmed_stats = topmed_stats.annotate(
        topmed=(topmed_stats.n_topmed_singletons / topmed_stats.n_singletons) > cutoff
    )
    return ht.annotate(topmed=topmed_stats[ht.key].topmed)


def main(args):
    subpop_nfe_ht = hl.read_table(subpop_ht_path("nfe")).select(
        "subpop", "known_subpop"
    )
    subpop_eas_ht = hl.read_table(subpop_ht_path("eas")).select(
        "subpop", "known_subpop"
    )
    ...
    exome_ht_c = exome_ht_c.transmute(
        **{f"PC{i+1}": exome_ht_c.scores[i] for i in range(10)}
    ).drop("qc_platform")

    genome_ht_a = hl.read_table(qc_ht_path("genomes", "hard_filters"))
    genome_ht_c = hl.read_table(qc_ht_path("genomes", "pop_platform"))
    genome_ht_c = genome_ht_c.transmute(
        **{f"PC{i+1}": genome_ht_c.scores[i] for i in range(10)}
    ).drop("qc_platform")

    super_meta_exome_ht = exome_ht_a.annotate(
        **exome_ht_b[exome_ht_a.key],
    ...
    super_meta_exome_ht = add_topmed_annotation(
        super_meta_exome_ht, "exomes", args.exomes_topmed_dup_cutoff
    )
    super_meta_exome_ht.write(
        metadata_exomes_ht_path(args.metadata_version_label), args.overwrite
    )
    super_meta_exome_ht.flatten().export(
        metadata_exomes_tsv_path(args.metadata_version_label)
    )

    super_meta_genome_ht = genome_ht_a.annotate(
        **genome_ht_c[genome_ht_a.key], **subpop_ht[genome_ht_a.key]
    )
    ...
    super_meta_genome_ht = add_topmed_annotation(
        super_meta_genome_ht, "genomes", args.genomes_topmed_dup_cutoff
    )
    super_meta_genome_ht.write(
        metadata_genomes_ht_path(args.metadata_version_label), args.overwrite
    )
    super_meta_genome_ht.flatten().export(
        metadata_genomes_tsv_path(args.metadata_version_label)
    )

    logger.info(
        "{0} exome, {1} genome high-quality samples remaining after sample QC".format(
            super_meta_exome_ht.aggregate(
                hl.agg.count_where(super_meta_exome_ht.high_quality)
            ),
            super_meta_genome_ht.aggregate(
                hl.agg.count_where(super_meta_genome_ht.high_quality)
            ),
        )
    )
    logger.info(
        "{0} exome, {1} genome release samples remaining after sample QC".format(
            super_meta_exome_ht.aggregate(
                hl.agg.count_where(super_meta_exome_ht.release)
            ),
            super_meta_genome_ht.aggregate(
                hl.agg.count_where(super_meta_genome_ht.release)
            ),
        )
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Label for metadata version (usually YYYY-MM-DD)",
