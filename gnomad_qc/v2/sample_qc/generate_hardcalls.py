...
def main(args):
    ...
    if args.write_hardcalls:
        ...
        mt = mt.select_entries(
            ...
            .when(hl.call(mt.PGT[0], mt.PGT[1]) == mt.GT, mt.PGT)
            .default(mt.GT),
            PID=mt.PID,
            adj=mt.adj,
        )
        ...
        mt.write(
            get_gnomad_data_path(data_type, hardcalls=True, split=False), args.overwrite
        )
    ...
    if args.split_hardcalls:
        ...
        mt.write(
            get_gnomad_data_path(data_type, hardcalls=True, split=True), args.overwrite
        )

    if args.write_nonrefs:  # CPU-hours: 600 (E)
        mt = get_gnomad_data(
            data_type, split=False, raw=True, meta_root=None
        ).select_cols()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--write_hardcalls", help="Creates a hardcalls mt", action="store_true"
    )
    parser.add_argument(
        "--split_hardcalls",
        help="Creates a split hardcalls mt from the hardcalls mt",
        action="store_true",
    )
    parser.add_argument(
        "--write_nonrefs",
        help="Creates a sparse mt with only non-ref genotypes",
        action="store_true",
    )
    parser.add_argument(
        "--split_nonrefs",
