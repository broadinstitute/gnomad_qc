...
from gnomad_qc.v2.sample_qc.joint_sample_qc import (
    run_assign_population_pcs,
    split_mt_by_relatedness,
)
...
def get_known_populations(
    ht: hl.Table, pop: str
):  # TODO: bring data into separate file and load here
...
def main(args):
    ...
    if not args.skip_filtering:
        ...
        exome_project_table = hl.read_table(
            qc_ht_path("exomes", "hard_filters")
        ).select("project_id")
        exome_platform_table = hl.read_table(qc_ht_path("exomes", "platforms")).select(
            "qc_platform"
        )
        exome_table = exome_project_table.annotate(
            qc_platform=hl.str(
                exome_platform_table[exome_project_table.key].qc_platform
            )
        )
        genome_table = hl.read_table(qc_ht_path("genomes", "hard_filters")).select(
            "project_id", "qc_platform"
        )
        joint_table = exome_table.union(genome_table)
        exome_pop_table = hl.read_table(qc_ht_path("exomes", "pop_platform")).select(
            "pop"
        )
        genome_pop_table = hl.read_table(qc_ht_path("genomes", "pop_platform")).select(
            "pop"
        )
        ...
        if args.population == "all":
            ...
            sample_criteria = (pruned_mt.meta.pop == "nfe") | (
                pruned_mt.meta.pop == "fin"
            )
        elif args.population == "eas":
            sample_criteria = (pruned_mt.meta.pop == "eas") & (
                pruned_mt.data_type == "exomes"
            )
        else:
        ...
        pca_platforms_mt = pca_platforms_mt.annotate_entries(
            remove_variant=(
                hl.case()
                .when(
                    pca_platforms_mt.total > 1000,
                    pca_platforms_mt.missing / pca_platforms_mt.total > 0.001,
                )
                .default(pca_platforms_mt.missing > 1)
            )
        )
        pca_platforms_mt = pca_platforms_mt.filter_rows(
            hl.agg.any(pca_platforms_mt.remove_variant), keep=False
        )
    ...
    if not args.skip_pop_pca:
        ...
        pca_loadings.write(
            ancestry_pca_loadings_ht_path(args.population), args.overwrite
        )
    ...
    joint_ht = joint_ht.select(
        *pop_colnames, **{f"PC{i + 1}": joint_ht.scores[i] for i in range(10)}
    )
    ...
    joint_pca_ht = joint_pca_ht.annotate(
        pop=hl.cond(
            joint_pca_ht.pop == "oth",
            hl.literal(f"o{args.population[:2]}"),
            joint_pca_ht.pop,
        )
    )
    joint_ht = joint_ht.select(
        **{f"subpop_{args.population}_PC{i}": joint_ht[f"PC{i}"] for i in range(1, 11)},
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--skip_filtering",
        help="Skip calculating filtered joint MT",
        action="store_true",
    )
    parser.add_argument(
        "--skip_pop_pca",
