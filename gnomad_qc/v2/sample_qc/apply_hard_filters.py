...
def make_hard_filters_expr(ht: hl.Table, data_type: str) -> hl.expr.SetExpression:
    ...
    if data_type == "exomes":
        ...
        hard_filters.update(
            {"coverage": ht.mean_dp < 15, "insert_size": ht.median_insert_size < 250}
        )
    return hl.set(
        hl.filter(
...
def main(args):
    ...
    qc_ht = annotate_sex(
        ...
        male_threshold=0.8 if args.genomes else 0.6,
    ).cols()
    ...
    checkpoint1b = qc_ht.aggregate(
        hl.agg.count_where(
            (hl.len(qc_ht["hard_filters"]) == 0) & (hl.len(qc_ht.perm_filters) == 0)
        )
    )
    logger.info("{} samples found before filtering".format(sample_count))
    logger.info(
        "{} samples found after checkpoint 1a (hard filters)".format(checkpoint1a)
    )
    logger.info(
        "{} samples found after checkpoint 1b (hard filters + permissions)".format(
            checkpoint1b
        )
    )
