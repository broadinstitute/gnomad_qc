...
def main(args):
    ...
    if args.subsample:
        ...
        logger.info(
            "Keeping {0} trios out of {1} from {2}".format(
                keep_count, total_count, data_type
            )
        )
        ...
        mt = mt.annotate_cols(
            is_related=relateds.contains(mt.s),
            keep_related=mt.meta.high_quality & keep_relateds.contains(mt.s),
        )
        ...
        mt.write(
            "gs://gnomad/variant_qc/temp/gnomad.{0}.{1}_samples.mt".format(
                data_type, N
            ),
            overwrite=True,
        )
        ...
        mt = hl.read_matrix_table(
            "gs://gnomad/variant_qc/temp/gnomad.{0}.{1}_samples.mt".format(data_type, N)
        )
        v, s = mt.count()
        logger.info(
            "{0} samples and {1} variants found in subsampled callset".format(s, v)
        )
        ht = mt.rows().repartition(1000, shuffle=False)
        ht.write(
            "gs://gnomad/variant_qc/temp/gnomad.{0}.{1}_samples.ht".format(
                data_type, N
            ),
            overwrite=True,
        )
    ...
    if args.add_vqsr_rank:
        ...
        ht = hl.read_table(
            "gs://gnomad/variant_qc/temp/gnomad.{0}.{1}_samples.ht".format(data_type, N)
        )
        ht = ht.key_by("locus", "alleles")
        ht = add_rank(ht, ht.info.VQSLOD)
        ht.write(
            "gs://gnomad/variant_qc/temp/gnomad.{0}.{1}_samples.vqsr.ranked.ht".format(
                data_type, N
            ),
            overwrite=True,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--sample_size",
        help="Total number of samples desired in subsample",
        default=10000,
        type=int,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--subsample", help="Subsample original callset.", action="store_true"
    )
    parser.add_argument(
        "--add_vqsr_rank", help="Add variant rankings.", action="store_true"
    )
