...
def create_vqsr_rank_ht(data_type: str):
    ...
    if not hl.utils.hadoop_exists(
        f"gs://gnomad-tmp/gnomad_{data_type}_vqsr_pre_ranking.ht/_SUCCESS"
    ):
        ...
        ht.write(
            f"gs://gnomad-tmp/gnomad_{data_type}_vqsr_pre_ranking.ht", overwrite=True
        )
...
def get_gnomad_annotations(data_type: str) -> hl.Table:
    ...
    """
    if not hl.utils.hadoop_exists(
        f"gs://gnomad/variant_qc/temp/gnomad_{data_type}_annotations.ht"
    ):
        ht = get_gnomad_data(data_type).rows()
        ...
        ht.repartition(1000, shuffle=False).write(
            f"gs://gnomad/variant_qc/temp/gnomad_{data_type}_annotations.ht"
        )
...
def create_cnn_rank_file() -> None:
    ...
    ht = ht.annotate(
        alt_alleles=ht.Alt.split(","), was_split=ht.Alt.contains(",")
    )  # This transforms to a list
...
def create_rf_2_0_2_rank(data_type: str, beta: bool) -> None:
    ...
    if not hl.hadoop_exists(
        ...
        ht = hl.import_table(
            get_2_0_2_rf_path(data_type, beta),
            types={"chrom": hl.tstr},
            impute=True,
            min_partitions=1000,
        )
        if "chrom" in ht.row:
            ...
            ht = ht.transmute(
                v=hl.parse_variant(ht.v), rfprob=ht.rf_rpob_tp  # Yes, this is awful
            )
    ...
    ht.write(
        score_ranking_path(data_type, "rf_2.0.2{}".format("_beta" if beta else "")),
        overwrite=True,
    )


def create_binned_data(
    ht: hl.Table, data: str, data_type: str, n_bins: int
) -> hl.Table:
    """
    ...
    return (
        ...
        .aggregate(
            ...
            n_trans_singletons=hl.agg.filter(
                (ht.info_ac < 3) & (ht.family_stats.unrelated_qc_callstats.AC[1] == 1),
...
def main(args):
    def run_data(
        ...
        if args.create_binned_file:
            ...
            binned_ht.write(
                score_ranking_path(data_type, score_type, binned=True), overwrite=True
            )
    ...
    for data_type in data_types:
        ...
        if args.rf_2_0_2_beta:
            run_data(
                create_rf_2_0_2_rank, [data_type, True], "rf_2.0.2_beta", data_type
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--rf_2_0_2",
        help="When set, creates the 2.0.2 RF rank file.",
        action="store_true",
    )
    parser.add_argument(
        "--rf_2_0_2_beta",
