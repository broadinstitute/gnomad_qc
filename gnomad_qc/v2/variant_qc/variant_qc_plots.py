...
import numpy as np
import pandas as pd
from bokeh.models import LabelSet, NumeralTickFormatter, Plot, Row, Span
...
def get_point_size_col(data: pd.Series, size_prop: str) -> pd.Series:
    ...
    if size_prop is None:
        ...
        if size_prop == "radius":
            ...
            raise ValueError(
                f"{size_prop} is not a supported value for argument `size_prop`"
            )
...
def get_binned_models_pd(
    data_type: str, models: Union[Dict[str, str], List[str]], contigs: Set[str] = None
) -> pd.DataFrame:
    """
    ...
    hts = [
        aggregate_contig(
            hl.read_table(score_ranking_path(data_type, model_id, binned=True)), contigs
...
def get_binned_concordance_pd(
    data_type: str, truth_samples: List[str], models: Union[Dict[str, str], List[str]]
) -> pd.DataFrame:
    """
    ...
    def get_binned_concordance_ht(
        data_type: str, truth_samples: List[str], models: Dict[str, str]
    ) -> hl.Table:
        """
        ...
        for truth_sample in truth_samples:
            for model_id, model_name in models.items():
                ht = hl.read_table(
                    binned_concordance_path(data_type, truth_sample, model_id)
                )
