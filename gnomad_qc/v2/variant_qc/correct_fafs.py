...
def main(args):
    ...
    for tag in freq_tables:
        ...
        freq_ht = freq_ht.annotate(
            ac1_pops=hl.zip_with_index(freq_ht.freq.map(lambda x: x.AC == 1))
            .filter(lambda x: x[1])
            .map(lambda x: x[0])
            .map(lambda x: hl.set(freq_ht.freq_meta[x].values()))
        )
        freq_ht = freq_ht.annotate(
            orig_faf95=hl.Struct(**make_faf_struct(freq_ht, faf_index_dict))
        )
        freq_ht = freq_ht.annotate(
            faf=freq_ht.faf.map(
                lambda x: hl.cond(
                    freq_ht.ac1_pops.contains(hl.set(x.meta.values())),
                    x.annotate(faf95=0, faf99=0),
                    x,
                )
            )
        )
        freq_ht = freq_ht.annotate(
            new_faf95=hl.Struct(**make_faf_struct(freq_ht, faf_index_dict))
        )
        ...
        # Write and overwrite
        freq_ht.drop("faf_ac", "ac1_pops", "orig_faf95", "new_faf95").write(
            annotations_ht_path(data_type, tag + "_faf_corrected"), args.overwrite
        )
        new_freq_ht = hl.read_table(
            annotations_ht_path(data_type, tag + "_faf_corrected")
        )
