...
def flag_problematic_regions(ht: hl.Table) -> hl.Table:
    ...
    ht = ht.annotate(
        ...
        decoy=hl.is_defined(decoy_intervals[ht.locus]),
        segdup=hl.is_defined(segdup_intervals[ht.locus]),
...
def sample_sum_check(
    ...
    combo_nhomalt = [
        ht.info[f"{prefix}_nhomalt_{x}"] for x in make_label_combos(label_groups)
    ]
...
def sanity_check_ht(
    ht: hl.Table, data_type, subsets, missingness_threshold=0.5, verbose=False
):
    """
    ...
    ht = ht.annotate(
        ...
        in_problematic_region=hl.any(
            lambda x: x, [ht.info.lcr, ht.info.segdup, ht.info.decoy]
        ),
    )
    ...
    for subfield in ["AC", "AN", "AF"]:
        ...
        generic_field_check(
            ht,
            (ht.info[f"gnomad_{subfield}_raw"] <= 0),
            f"gnomad {subfield}_raw > 0",
            [f"info.gnomad_{subfield}_raw"],
            verbose,
        )
        generic_field_check(
            ht,
            (ht.info[f"gnomad_{subfield}_adj"] < 0),
            f"gnomad {subfield}_adj >= 0",
            [f"info.gnomad_{subfield}_adj", "filters"],
            verbose,
        )
    ...
    for subset in subsets:
        for subfield in ["AC", "AN", "nhomalt"]:
            ...
            generic_field_check(
                ...
                [f"info.{subset}_{subfield}_raw", f"info.{subset}_{subfield}_adj"],
                verbose,
            )
    ...
    for subset in subsets:
        ...
        sample_sum_check(
            ht, subset, dict(group=["adj"], pop=pop_adjusted, sex=SEXES), verbose
        )
        ...
        nfe_subpop_adjusted = list(
            set(
                [
                    ...
                    if subset in x and "nfe_" in x and "male" not in x
                ]
            )
        )
        if nfe_subpop_adjusted != []:
            sample_sum_check(
                ht,
                subset,
                dict(group=["adj"], pop=["nfe"], subpop=nfe_subpop_adjusted),
                verbose,
                subpop="nfe",
            )
        if data_type == "exomes":
            ...
            if eas_subpop_adjusted != []:
                sample_sum_check(
                    ht,
                    subset,
                    dict(group=["adj"], pop=["eas"], subpop=eas_subpop_adjusted),
                    verbose,
                    subpop="eas",
                )
...
def make_freq_meta_index_dict(freq_meta):
    ...
    index_dict.update(
        index_globals(freq_meta, dict(group=GROUPS, pop=["nfe"], subpop=NFE_SUBPOPS))
    )
    index_dict.update(
        index_globals(freq_meta, dict(group=GROUPS, pop=["eas"], subpop=EAS_SUBPOPS))
    )
    return index_dict
...
def unfurl_nested_annotations(ht):
    ...
    for k, i in hl.eval(
        ht.globals.faf_index_dict
    ).items():  # NOTE: faf annotations are all done on adj-only groupings
        entry = k.split("_")
        if entry[0] == "non":
            prefix = "_".join(entry[:2])
            combo_fields = ["adj"] + entry[2:]
        ...
        combo_dict = {
            f"{prefix}_faf95_{combo}": hl.or_missing(
                hl.set(ht.faf[i].meta.values()) == set(combo_fields), ht.faf[i].faf95
            ),
            f"{prefix}_faf99_{combo}": hl.or_missing(
                hl.set(ht.faf[i].meta.values()) == set(combo_fields), ht.faf[i].faf99
            ),
        }
        expr_dict.update(combo_dict)
    ...
    for prefix, i in hl.eval(ht.globals.popmax_index_dict).items():
        ...
        if prefix == "gnomad":
            age_hist_dict = {
                f"{prefix}_age_hist_het_bin_freq": hl.delimit(
                    ht.age_hist_het[i].bin_freq, delimiter="|"
                ),
                f"{prefix}_age_hist_het_bin_edges": hl.delimit(
                    ht.age_hist_het[i].bin_edges, delimiter="|"
                ),
                f"{prefix}_age_hist_het_n_smaller": ht.age_hist_het[i].n_smaller,
                f"{prefix}_age_hist_het_n_larger": ht.age_hist_het[i].n_larger,
                f"{prefix}_age_hist_hom_bin_freq": hl.delimit(
                    ht.age_hist_hom[i].bin_freq, delimiter="|"
                ),
                f"{prefix}_age_hist_hom_bin_edges": hl.delimit(
                    ht.age_hist_hom[i].bin_edges, delimiter="|"
                ),
...
def make_hist_bin_edges_expr(ht):
    ...
    edges_dict = {
        "gnomad_het": "|".join(
            map(lambda x: f"{x:.1f}", ht.take(1)[0].age_hist_het[0].bin_edges)
        ),
        "gnomad_hom": "|".join(
            map(lambda x: f"{x:.1f}", ht.take(1)[0].age_hist_hom[0].bin_edges)
        ),
    }
    for hist in HISTS:
        edges_dict[hist] = (
            "|".join(map(lambda x: f"{x:.2f}", ht.take(1)[0][hist].bin_edges))
            if "ab" in hist
            else "|".join(map(lambda x: str(int(x)), ht.take(1)[0][hist].bin_edges))
        )
    return edges_dict
...
def set_female_y_metrics_to_na(ht):
    ...
    for metric in female_metrics:
        female_metrics_dict.update(
            {
                f"{metric}": hl.cond(
                    ht.locus.contig == "Y", hl.null(hl.tint32), ht.info[f"{metric}"]
                )
            }
        )
...
def build_new_index_dict(ht, subsets, array_lengths):
    ...
    for subset in subsets:
        ...
        new_index_dict.update(
            {subset + "_" + k: v + n for k, v in temp_index_dict.items()}
        )
        n += array_lengths[i]
...
def concat_struct_expr(ht, subsets, field):
    ...
    if len(copy_subsets) == 0:
        ...
        expr = concat_struct_expr(ht, copy_subsets, field).extend(
            hl.array([ht[subset][field]])
        )
    return expr
...
def make_faf_index_dict(
    ht, subset=None
):  # NOTE: label groups: assumes 'adj', plus all pops
    """
...
def build_faf_index_dict(ht, subsets):
    ...
    for subset in subsets:
        ...
        new_index_dict.update(
            {subset + "_" + k: v + n for k, v in temp_faf_index.items()}
        )
        n += len(temp_faf_index)
    return new_index_dict
...
def get_age_distributions(data_type):
    ...
    age_hist_data = meta.aggregate(
        hl.agg.filter(meta.release, hl.agg.hist(meta.age, 30, 80, 10))
    )
...
def main(args):
    ...
    import_ht_path = (
        release_ht_path(data_type)
        if args.include_subset_frequencies
        else release_ht_path(data_type, with_subsets=False)
    )
    age_hist_data = get_age_distributions(data_type)

    if args.prepare_internal_ht:
        freq_ht = hl.read_table(
            annotations_ht_path(data_type, "frequencies_with_consanguineous")
        )  # FIXME: revert to plain 'frequencies' for v3+
        index_dict = make_index_dict(freq_ht)

        rf_ht = hl.read_table(annotations_ht_path(data_type, "rf")).drop(
            "info_ac", "ac", "ac_raw"
        )
        ...
        ht = prepare_table_annotations(
            freq_ht, rf_ht, vep_ht, dbsnp_ht, hist_ht, index_dict, allele_ht
        )
    ...
    if args.add_subset_frequencies:
        ...
        controls_ht = hl.read_table(
            annotations_ht_path(data_type, "frequencies_control_with_consanguineous")
        )  # FIXME: revert to plain 'frequencies' for v3+
        freq_index_dict = make_index_dict(controls_ht)
        print(hl.eval(controls_ht.freq_meta))
        controls_ht = controls_ht.drop("helloworld")

        non_neuro_ht = hl.read_table(
            annotations_ht_path(data_type, "frequencies_neuro_with_consanguineous")
        )  # FIXME: revert to plain 'frequencies' for v3+
        non_topmed_ht = hl.read_table(
            annotations_ht_path(data_type, "frequencies_topmed_with_consanguineous")
        )  # FIXME: revert to plain 'frequencies' for v3+
        ...
        combined_ht = ht.annotate(
            controls=controls_ht[ht.key],
            non_neuro=non_neuro_ht[ht.key],
            non_topmed=non_topmed_ht[ht.key],
        )
        combined_ht = combined_ht.annotate_globals(
            controls_index_dict=make_index_dict(controls_ht),
            non_neuro_index_dict=make_index_dict(non_neuro_ht),
            non_topmed_index_dict=make_index_dict(non_topmed_ht),
            freq_meta=combined_ht.freq_meta.extend(controls_freq_meta)
            .extend(non_neuro_freq_meta)
            .extend(non_topmed_freq_meta),
        )

        if data_type == "exomes":
            non_cancer_ht = hl.read_table(
                annotations_ht_path(data_type, "frequencies_tcga_with_consanguineous")
            )
            ...
            combined_ht = combined_ht.annotate(
                non_cancer=non_cancer_ht[combined_ht.key]
            )
            combined_ht = combined_ht.annotate_globals(
                non_cancer_index_dict=make_index_dict(non_cancer_ht),
        ...
        subsets = (
            ...
            if args.exomes
            else ["controls", "non_neuro", "non_topmed"]
        )
        array_lengths = get_array_lengths(combined_ht, subsets)
        combined_ht = combined_ht.select_globals(
            "rf",
            "freq_meta",
            freq_index_dict={
                k.replace("_adj", ""): v
                for k, v in build_new_index_dict(
                    combined_ht, subsets, array_lengths
                ).items()
            },
            popmax_index_dict={v: i for i, v in enumerate(["gnomad"] + subsets)},
            age_index_dict={v: i for i, v in enumerate(["gnomad"] + subsets)},
            faf_index_dict={
                k.replace("_adj", ""): v
                for k, v in build_faf_index_dict(combined_ht, subsets).items()
            },
        )
        combined_ht = combined_ht.transmute(
            freq=concat_array_expr(combined_ht, subsets, "freq"),
    ...
    if args.prepare_release_vcf:
        ...
        if data_type == "exomes":
            subset_list = (
                ["gnomad", "controls", "non_neuro", "non_cancer", "non_topmed"]
                if args.include_subset_frequencies
                else ["gnomad"]
            )
        else:
        ...
        for subset in subset_list:
            ...
            INFO_DICT.update(
                make_info_dict(subset, dict(group=GROUPS, pop=POPS, sex=SEXES))
            )
            INFO_DICT.update(
                make_info_dict(
                    subset, dict(group=GROUPS, pop=["nfe"], subpop=NFE_SUBPOPS)
                )
            )
            INFO_DICT.update(
                make_info_dict(
                    subset, dict(group=GROUPS, pop=["eas"], subpop=EAS_SUBPOPS)
                )
            )
        ...
        new_row_annots = [
            x.replace("gnomad_", "").replace("_adj", "") for x in row_annots
        ]
        ...
        header_dict = {"info": new_info_dict, "filter": make_filter_dict(ht)}
        ht = ht.annotate(info=ht.info.annotate(vep=vep_csq_ht[ht.key].vep))

        # Export VCFs, full and by chromosome
        gnomad_ref = hl.ReferenceGenome.read(
            "gs://gnomad-public/resources/gnomad_grch37.json"
        )
        ht = ht.key_by(
            locus=hl.locus(
                ht.locus.contig, ht.locus.position, reference_genome=gnomad_ref
            ),
            alleles=ht.alleles,
        )

        for contig in contigs:
            contig_ht = hl.filter_intervals(
                ht, [hl.parse_locus_interval(contig, reference_genome=gnomad_ref)]
            )
            mt = hl.MatrixTable.from_rows_table(contig_ht).key_cols_by(s="foo")
            hl.export_vcf(
                mt, release_vcf_path(data_type, contig=contig), metadata=header_dict
            )
        ...
        if data_type == "genomes":
            intervals = hl.import_locus_intervals(
                ...
                reference_genome=gnomad_ref,
            )
            coding_mt = mt.filter_rows(hl.is_defined(intervals[mt.locus]), keep=True)
            hl.export_vcf(
                coding_mt,
                release_vcf_path(data_type, coding_only=True),
    ...
    if args.sanity_check_sites:
        ...
        ht = hl.read_table(release_ht_path(data_type, nested=False, temp=True))
        sanity_check_ht(
            ht, data_type, subset_list, missingness_threshold=0.5, verbose=args.verbose
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--prepare_internal_ht", help="Prepare internal HailTable", action="store_true"
    )
    parser.add_argument(
        "--add_subset_frequencies",
        help="Add frequency annotations for gnomAD subsets",
        action="store_true",
    )
    parser.add_argument(
        "--include_subset_frequencies",
        help="Include frequency annotations for gnomAD subsets in release",
        action="store_true",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--sanity_check_sites", help="Run sanity checks function", action="store_true"
    )
    parser.add_argument(
        "--liftover", help="Liftover final sites file", action="store_true"
    )
    parser.add_argument(
        "--verbose",
