...
def define_hist_ranges(ht):
    hist_dict = {
        "FS": hl.agg.hist(
            ht.FS, 0, 50, 50
        ),  # NOTE: in 2.0.2 release this was on (0,20)
        ...
        "DP": hl.agg.hist(
            hl.log(ht.DP, base=10), 1, 9, 32
        ),  # NOTE: in 2.0.2 release this was on (0,8)
        "VQSLOD": hl.agg.hist(
            ht.VQSLOD, -30, 30, 60
        ),  # NOTE: in 2.0.2 release this was on (-20,20)
...
def aggregate_qual_stats_by_bin(ht):
    ht = ht.annotate(
        metric=(
            ...
            .when(
                (ht.gnomad_AC_raw > 2) & (ht.gnomad_AF_raw < 0.00005), "binned_0.00005"
            )
            .when(
                (ht.gnomad_AF_raw >= 0.00005) & (ht.gnomad_AF_raw < 0.0001),
                "binned_0.0001",
            )
            .when(
                (ht.gnomad_AF_raw >= 0.0001) & (ht.gnomad_AF_raw < 0.0002),
                "binned_0.0002",
            )
            .when(
                (ht.gnomad_AF_raw >= 0.0002) & (ht.gnomad_AF_raw < 0.0005),
                "binned_0.0005",
            )
            .when(
                (ht.gnomad_AF_raw >= 0.0005) & (ht.gnomad_AF_raw < 0.001),
                "binned_0.001",
            )
            .when(
                (ht.gnomad_AF_raw >= 0.001) & (ht.gnomad_AF_raw < 0.002), "binned_0.002"
            )
            .when(
                (ht.gnomad_AF_raw >= 0.002) & (ht.gnomad_AF_raw < 0.005), "binned_0.005"
            )
            .when(
                (ht.gnomad_AF_raw >= 0.005) & (ht.gnomad_AF_raw < 0.01), "binned_0.01"
            )
