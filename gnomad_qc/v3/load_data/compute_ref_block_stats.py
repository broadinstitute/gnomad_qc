...
def print_ref_block_stats(path: str):
    ...
    def _print_block_stats(stats: hl.Struct):
        ...
        for strat, ref_block_stats in [
            ("all", stats.ref_block_stats),
            ("adj", stats.adj_ref_block_stats),
        ]:
            n_blocks = (
                np.sum(ref_block_stats.hist.bin_freq) + ref_block_stats.hist.n_larger
            )
        ...
        hl.struct(
            ref_block_stats=hl.struct(
                ...
                hist_log=hl.agg.hist(
                    hl.log10(1 + mt.END - mt.locus.position), 0, 5, 100
                ),
            ),
...
def main(args):  # noqa: D103
    if args.compute_stats:
        ...
        print_ref_block_stats(
            "gs://gnomad/annotations/hail-0.2/ht/genomes_v3/gnomad_genomes_v3.repartitioned.ref_block_stats.pickle"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--compute_stats", help="Lift-over Purcell5k intervals", action="store_true"
    )
    parser.add_argument(
        "--stats_path",
        help="Path to stats file",
        default="gs://gnomad/annotations/hail-0.2/ht/genomes_v3/gnomad_genomes_v3.repartitioned.ref_block_stats.pickle",
    )
    parser.add_argument(
        "--print_stats", help="Print existing stats", action="store_true"
    )
