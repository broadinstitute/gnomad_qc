...
def main(args):
    if args.join_qc_mt:
        ...
        v2_qc_mt_liftover = v2_qc_mt_liftover.key_cols_by(
            s=v2_qc_mt_liftover.s, data_type="v2_exomes"
        )
        v3_qc_mt = qc.mt()
        ...
        joint_qc_mt.write(
            "gs://gnomad-tmp/v2_exomes_v3_joint_qc.mt", overwrite=args.overwrite
        )
    ...
    if args.run_pc_relate:
        ...
        eig, scores, _ = hl.hwe_normalized_pca(
            joint_qc_mt.GT, k=10, compute_loadings=False
        )
        scores = scores.checkpoint(
            v2_v3_pc_relate_pca_scores.path,
        ...
        relatedness_ht = hl.pc_relate(
            ...
            scores_expr=scores[joint_qc_mt.col_key].scores,
            block_size=4096,
            min_kinship=0.1,
            statistics="all",
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--join_qc_mt",
        help="Join liftover v2 exomes release and v3 release QC MT",
        action="store_true",
    )
    parser.add_argument(
        "--run_pc_relate",
        help="Run PC-relate on v2 exomes/v3 combined release QC MT",
        action="store_true",
    )
