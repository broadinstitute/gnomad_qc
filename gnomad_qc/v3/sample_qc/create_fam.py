...
def run_mendel_errors() -> hl.Table:  # noqa: D103
    meta_ht = meta.ht()
    ped = pedigree.versions[f"{CURRENT_VERSION}_raw"].pedigree()
...
def main(args):  # noqa: D103
    ...
    if args.infer_families:
        ...
        ped.write(pedigree.versions[f"{CURRENT_VERSION}_raw"].path)
        raw_trios = families_to_trios(ped)
        raw_trios.write(trios.versions[f"{CURRENT_VERSION}_raw"].path)
    ...
    if args.finalize_ped:
        final_ped = filter_ped(
            pedigree.versions[f"{CURRENT_VERSION}_raw"].pedigree,
