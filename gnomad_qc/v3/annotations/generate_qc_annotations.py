...
def generate_ac(mt: hl.MatrixTable, fam_file: str) -> hl.Table:
    ...
    mt = mt.annotate_rows(
        ...
        ac_qc_samples_unrelated_adj=hl.agg.filter(
            ~mt.meta.all_samples_related & mt.adj, hl.agg.sum(mt.GT.n_alt_alleles())
        ),
...
def generate_fam_stats(mt: hl.MatrixTable, fam_file: str) -> hl.Table:
    ...
    parents_high_depth = (mt.mother_entry.AD[0] + mt.mother_entry.AD[1] > 20) & (
        mt.father_entry.AD[0] + mt.father_entry.AD[1] > 20
    )
    ...
    ht = mt.select_rows(
        **generate_trio_stats_expr(
            mt,
            transmitted_strata={"raw": None, "adj": trio_adj},
            de_novo_strata={
                "raw": None,
                "adj": trio_adj,
...
def export_transmitted_singletons_vcf():
    ...
    for transmission_confidence in ["raw", "adj"]:
        ...
        hl.export_vcf(
            ts_mt, get_transmitted_singleton_vcf_path(transmission_confidence)
        )
...
def run_vep() -> hl.Table:
    def get_mt_partitions(mt_path: str) -> List[hl.Interval]:
        ...
        with hl.hadoop_open(
            path.join(mt_path, "rows", "rows", "metadata.json.gz")
        ) as f:
            intervals_json = json.load(f)["jRangeBounds"]
            return hl.tarray(
                hl.tinterval(hl.tstruct(locus=mt.locus.dtype))
            )._convert_from_json(intervals_json)
