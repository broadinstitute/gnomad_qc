    ...
    return VersionedTableResource(
        CURRENT_SAMPLE_QC_VERSION,
        {
            version: TableResource(
                f"{get_sample_qc_root(version, test, data_set='aou', environment=environment)}/hard_filtering/aou.v{version}.sample_qc_all{'' if strat == 'all' else f'_{strat}'}.ht"
            )
...
def get_hard_filtered_samples(
    test: bool = False,
    environment: str = "rwb",
) -> VersionedTableResource:
    """
    Get the VersionedTableResource for hard-filtered samples.

    :param test: Whether to use a tmp path for a test resource.
    :param environment: Environment to use. Default is "rwb". Must be one of "rwb",
        "batch", or "dataproc".
    :return: VersionedTableResource of hard-filtered samples.
    """
    return VersionedTableResource(
        CURRENT_SAMPLE_QC_VERSION,
        {
            version: TableResource(
                f"{get_sample_qc_root(version, test, environment=environment)}/hard_filtering/aou.genomes.v{version}.hard_filtered_samples.ht"
            )
            for version in SAMPLE_QC_VERSIONS
        },
    )
...
hard_filtered_samples = get_hard_filtered_samples()
...
def get_aou_mt_union(
    ...
    :param test: Whether to use a tmp path for a test resource. Default is True.
    :param environment: Environment to use. Default is "rwb". Must be one of "rwb",
        "batch", or "dataproc".
    :return: MatrixTableResource containing the union of AoU ACAF and exome MTs.
    """
    return MatrixTableResource(
        path=f"{get_sample_qc_root(CURRENT_SAMPLE_QC_VERSION, test=test, data_type='genomes', data_set='aou', environment=environment)}/union_aou/aou.acaf.exomes.union.mt"
    )
    ...
    return VersionedMatrixTableResource(
        CURRENT_SAMPLE_QC_VERSION,
        {
            version: MatrixTableResource(
                f"{get_sample_qc_root(version, test, data_type='joint', data_set='aou', environment=environment)}/qc_mt/gnomad.joint.v{version}.qc.mt"
            )
            for version in SAMPLE_QC_VERSIONS
        },
    )
...
def relatedness(
    ...
    return VersionedTableResource(
        ...
        {
            version: TableResource(
                f"{get_sample_qc_root(version, test, data_type='joint', environment=environment)}/relatedness/gnomad.joint.v{version}.relatedness{suffix}.ht"
            )
            for version in SAMPLE_QC_VERSIONS
        },
    )
...
def related_samples_to_drop(
    ...
    return VersionedTableResource(
        ...
        {
            version: TableResource(
                f"{get_sample_qc_root(version, test, data_type='joint', environment=environment)}/relatedness/gnomad.joint.v{version}.related_samples_to_drop.{'release' if release else 'pca'}.ht"
            )
            for version in SAMPLE_QC_VERSIONS
        },
    )
...
def sample_rankings(
    ...
    return VersionedTableResource(
        ...
        {
            version: TableResource(
                f"{get_sample_qc_root(version, test, data_type='joint', environment=environment)}/relatedness/gnomad.joint.v{version}.samples_ranking.{'release' if release else 'pca'}.ht"
            )
            for version in SAMPLE_QC_VERSIONS
        },
    )
...
def _get_genetic_ancestry_pca_ht_path(
    ...
    """
    return f"{get_sample_qc_root(version, test, data_type, environment=environment)}/genetic_ancestry_inference/gnomad.{data_type}.v{version}.pca_{part}{'_with_unreleasable_samples' if include_unreleasable_samples else ''}.ht"
...
def gen_anc_rf_path(
    ...
    """
    return f"{get_sample_qc_root(version, test, data_type, environment=environment)}/genetic_ancestry_inference/gnomad.{data_type}.v{version}.gen_anc.RF_fit.pickle"
...
def get_gen_anc_ht(
    ...
    return TableResource(
        f"{get_sample_qc_root(version, test, data_type, environment=environment)}/genetic_ancestry_inference/gnomad.{data_type}.v{version}.gen_anc{'_projection_only' if projection_only else ''}.ht"
    )
...
def get_gen_anc_pr_ht(
    ...
    """
    return TableResource(
        f"{get_sample_qc_root(version, test, data_type, environment=environment)}/genetic_ancestry_inference/gnomad.{data_type}.v{version}.gen_anc_pr.ht"
    )


def per_grp_min_rf_probs_json_path(
    version: str = CURRENT_SAMPLE_QC_VERSION,
    environment: str = "rwb",
):
    """
    ...
    """
    return f"{get_sample_qc_root(version, data_type='joint', environment=environment)}/genetic_ancestry_inference/gnomad.joint.v{version}.gen_anc_min_probs.json"
...
def get_outlier_detection_sample_qc(
    ...
    return VersionedTableResource(
        ...
        {
            version: TableResource(
                f"{get_sample_qc_root(version, test, environment=environment)}/outlier_detection/gnomad.v{version}.outlier_detection_sample_qc.ht"
            )
            for version in SAMPLE_QC_VERSIONS
        },
    )
...
def stratified_filtering(
    ...
    return VersionedTableResource(
        ...
        {
            version: TableResource(
                f"{get_sample_qc_root(version, test, environment=environment)}/outlier_detection/gnomad.genomes.v{version}.stratified_filtering.gen_anc_stratified.ht"
            )
            for version in SAMPLE_QC_VERSIONS
        },
    )
...
def regressed_filtering(
    ...
    return VersionedTableResource(
        ...
        {
            version: TableResource(
                f"{get_sample_qc_root(version, test, environment=environment)}/outlier_detection/gnomad.genomes.v{version}.regressed_filtering.gen_anc_pc_regressed{postfix}.ht"
            )
            for version in SAMPLE_QC_VERSIONS
        },
    )
...
def nearest_neighbors(
    ...
    return VersionedTableResource(
        ...
        {
            version: TableResource(
                f"{get_sample_qc_root(version, test, environment=environment)}/outlier_detection/gnomad.genomes.v{version}.nearest_neighbors{postfix}.ht"
            )
            for version in SAMPLE_QC_VERSIONS
        },
    )
...
def nearest_neighbors_filtering(
    ...
    return VersionedTableResource(
        ...
        {
            version: TableResource(
                f"{get_sample_qc_root(version, test, environment=environment)}/outlier_detection/gnomad.genomes.v{version}.nearest_neighbors_filtering.ht"
            )
            for version in SAMPLE_QC_VERSIONS
        },
    )
...
def finalized_outlier_filtering(
    ...
    return VersionedTableResource(
        ...
        {
            version: TableResource(
                f"{get_sample_qc_root(version, test, environment=environment)}/outlier_detection/gnomad.genomes.v{version}.final_outlier_filtering.ht"
            )
            for version in SAMPLE_QC_VERSIONS
        },
    )
...
def duplicates(environment: str = "rwb") -> VersionedTableResource:
    """
...
def trios(
    fake: bool = False, test: bool = False, environment: str = "rwb"
) -> VersionedPedigreeResource:
    """
