...
All composition plots and the histogram use **passing bins only** — those whose training-fold sample count is at least `--min-bin-size` (default 50). Bins below the threshold are dropped, not relabeled. The script prints the total and passing training-sample counts and the number of passing groups at each hierarchy level to stdout.
