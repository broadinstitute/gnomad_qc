...
def get_per_sample_counts(
    ...
    return VersionedTableResource(
        ...
        {
            version: TableResource(
                f"{_release_root(version=version, test=test, data_type=data_type, extension='ht')}/gnomad.{data_type}.v{version}.{suffix if suffix else ''}per_sample_variant_counts{'_aggregated' if aggregated else ''}.ht"
            )
            for version in RELEASES
        },
    )
