...
def get_per_sample_counts(
    ...
    :param suffix: Additional name to append, containing some method or filtering information
    :param aggregated: Whether to use the aggregated per-sample counts table. Default is False.
    :param by_ancestry: Whether to use the per-sample counts table stratified by ancestry. Default is False.
    :param by_subset: Whether to use the per-sample counts table stratified by subset. Default is False.
    ...
    return VersionedTableResource(
        ...
        {
            version: TableResource(
                f"{_release_root(version=version, test=test, data_type=data_type, extension='ht')}/gnomad.{data_type}.v{version}.{suffix if suffix else ''}per_sample_variant_counts{'_aggregated' if aggregated else ''}{'_by_ancestry' if by_ancestry else ''}{'_by_subset' if by_subset else ''}.ht"
            )
            for version in RELEASES
        },
    )
