    ...
    return VersionedTableResource(
        CURRENT_VERSION,
        {
            version: TableResource(
                f"{get_sample_qc_root(version, test)}/hard_filtering/gnomad.exomes.v{version}.sample_qc_all_{'' if strat == 'all' else strat}.ht"
            )
...
def _get_ancestry_pca_ht_path(
    ...
    """
    return (
        f"{get_sample_qc_root(version)}/ancestry_inference/gnomad.exomes.v{version}.pca_{part}{'_with_unreleasable_samples' if include_unreleasable_samples else ''}.ht"
    )
...
pop = VersionedTableResource(
    ...
    {
        version: TableResource(
            f"{get_sample_qc_root(version)}/ancestry_inference/gnomad.exomes.v{version}.pop.ht"
        )
        for version in VERSIONS
    },
)
...
def pop_tsv_path(version: str = CURRENT_VERSION) -> str:
    ...
    """
    return f"gs://gnomad/v{version}/sample_qc/exomes/ancestry_inference/gnomad.exomes.v{version}.RF_pop_assignments.txt.gz"
...
def pop_rf_path(version: str = CURRENT_VERSION) -> str:
    ...
    """
    return f"gs://gnomad/v{version}/sample_qc/exomes/ancestry_inference/gnomad.exomes.v{version}.pop.RF_fit.pickle"
