...
CURRENT_SUMMARY_STATS_VERSION = "4.0"
SUMMARY_STATS_VERSIONS = ["4.0"]


def get_summary_stats_root(
    version: str = CURRENT_SUMMARY_STATS_VERSION, test: bool = False, data_type="exomes"
) -> str:
    """
    Return path to summary stats root folder.

    :param version: Version of summary stats path to return
    ...
    :return: Root to summary stats path
    """
    return (
        f"gs://gnomad-tmp/gnomad_v{version}_testing/summary_stats/{data_type}"
        if test
        else f"gs://gnomad/v{version}/summary_stats/{data_type}"
    )


def get_per_sample_summary_stats(
    ...
    return VersionedTableResource(
        CURRENT_SUMMARY_STATS_VERSION,
        {
            version: TableResource(
                f"{get_summary_stats_root(version, test, data_type)}/gnomad.{data_type}.v{version}.summary_stats.per_sample_variant_counts.ht"
            )
            for version in SUMMARY_STATS_VERSIONS
        },
    )
...
def per_sample_datatype(
    ...
    sample_qc_ht = sample_qc_ht.checkpoint(
        get_per_sample_summary_stats(test=test, data_type=data_type).path,
        overwrite=overwrite,
    )
