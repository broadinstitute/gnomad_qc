...
def compute_sample_qc() -> hl.Table:
    ...
    mt = filter_to_autosomes(get_full_mt(key_by_locus_and_alleles=True))
    ...
    sample_qc_ht = compute_stratified_sample_qc(
        ...
        tmp_ht_prefix=sample_qc_path()[:-3]
    )
...
def compute_qc_mt() -> hl.MatrixTable:
    ...
    v2_qc_sites = hl.read_table(gnomad_v2_qc_sites_path).key_by('locus')
    p5k = hl.read_table(purcell_5k_path)
    qc_sites = v2_qc_sites.union(p5k, unify=True)

    lcr = get_lcr_intervals()
    qc_sites = qc_sites.filter(
        hl.is_missing(lcr[qc_sites.key])
    )
    ...
    mt = densify_sites(
        mt,
        qc_sites,
        hl.read_table(last_END_position_path)
    )
...
def compute_hard_filters(cov_threshold: int) -> hl.Table:
    ht = get_full_mt(remove_hard_filtered_samples=False).cols()
    ...
    cov_ht = hl.read_table(v3_sex_ht_path)
    ...
    sex_ht = hl.read_table(v3_sex_ht_path)[ht.key]
...
def compute_sex() -> hl.Table:
    ...
    ht = impute_sex_ploidy(
        get_full_mt(split=False, remove_hard_filtered_samples=False),
        excluded_intervals=hl.import_bed(telomeres_and_centromeres_bed_path, reference_genome='GRCh38')
    )
    ht = ht.checkpoint('gs://gnomad-tmp/sex_depth.ht', overwrite=True)

    # Compute F-stat
    chrom_x_ht = get_full_mt(split=False, key_by_locus_and_alleles=True, remove_hard_filtered_samples=False)
...
def compute_sample_rankings(use_qc_metrics_filters: bool) -> hl.Table:
    ...
    sex_ht = hl.read_table(v3_sex_ht_path)
    project_ht = get_project_meta()

    project_ht = project_ht.select(
        'releasable',
        chr20_mean_dp=sex_ht[project_ht.key].chr20_mean_dp,
        filtered=hl.or_else(hl.len(hard_filtered_samples_ht[project_ht.key].hard_filters)>0, False)
    )
    ...
    if use_qc_metrics_filters:
        ...
        project_ht = project_ht.annotate(
            filtered=hl.cond(
                ...
                True,
                hl.or_else(
                    hl.len(regressed_metrics_ht[project_ht.key].qc_metrics_filters)>0,
                    False
                )
            )
        )
...
def run_pca(
    ...
    if not include_unreleasable_samples:
        ...
        project_meta_ht = get_project_meta()
        samples_to_drop = samples_to_drop.union(
            qc_mt.filter_cols(~project_meta_ht[qc_mt.col_key].releasable).cols().select()
        )
    else:
...
def assign_pops(
    ...
    pop_pca_scores_ht = hl.read_table(get_ancestry_pca_ht_path('scores', include_unreleasable_samples))
    project_meta_ht = get_project_meta()

    project_meta_ht = project_meta_ht[pop_pca_scores_ht.key]
...
def apply_stratified_filters(filtering_qc_metrics: List[str]) -> hl.Table:
    ...
    sample_qc_ht = hl.read_table(sample_qc_path('bi_allelic'))
...
def apply_regressed_filters(
    ...
    sample_qc_ht = hl.read_table(sample_qc_path('bi_allelic'))
    pca_ht = hl.read_table(get_ancestry_pca_ht_path('scores', include_unreleasable_samples))
    project_meta_ht = get_project_meta()
    sample_qc_ht = sample_qc_ht.select(
        **sample_qc_ht.sample_qc,
        **pca_ht[sample_qc_ht.key],
...
def generate_metadata() -> hl.Table:
    meta_ht = get_project_meta().repartition(100)
    hard_filters_ht = hl.read_table(hard_filtered_samples_ht_path)
    regressed_metrics_ht = hl.read_table(regressed_metrics_ht_path)
    pop_ht = hl.read_table(pop_ht_path)
    release_related_samples_to_drop_ht = hl.read_table(release_related_samples_to_drop_ht_path)
    pca_related_samples_to_drop_ht = hl.read_table(pca_related_samples_to_drop_ht_path)
    sex_ht = hl.read_table(v3_sex_ht_path)
...
def main(args):
    ...
    if args.sample_qc:
        compute_sample_qc().write(sample_qc_path(), overwrite=args.overwrite)

    if args.compute_qc_mt:
        compute_qc_mt().write(v3_qc_mt_path, overwrite=args.overwrite)

    if args.impute_sex:
        compute_sex().write(v3_sex_ht_path, overwrite=args.overwrite)
    elif args.reannotate_sex:
        sex_ht = hl.read_table(v3_sex_ht_path)
        sex_ht = sex_ht.checkpoint('gs://gnomad-tmp/sex_ht_checkpoint.ht', overwrite=True) # Copy HT to temp location to overwrite annotations
        ...
        sex_ht.write(v3_sex_ht_path, overwrite=args.overwrite)

    if args.compute_hard_filters:
        compute_hard_filters(
            args.min_cov
        ).write(hard_filtered_samples_ht_path, overwrite=args.overwrite)
    ...
    if args.run_pc_relate:
        ...
        scores = scores.checkpoint(v3_pc_relate_pca_scores_ht_path, overwrite=args.overwrite, _read_if_exists=not args.overwrite)
        ...
        relatedness_ht.write(v3_relatedness_ht_path, args.overwrite)

    if args.run_pca:
        rank_ht = compute_sample_rankings(use_qc_metrics_filters=False) # QC metrics filters do not exist at this point
        rank_ht = rank_ht.checkpoint(pca_samples_rankings_ht_path, overwrite=args.overwrite, _read_if_exists=not args.overwrite)
        ...
        samples_to_drop = compute_related_samples_to_drop(
            hl.read_table(v3_relatedness_ht_path),
            rank_ht,
            args.kin_threshold,
            filtered_samples=filtered_samples
        )
        samples_to_drop.checkpoint(pca_related_samples_to_drop_ht_path, overwrite=args.overwrite, _read_if_exists=not args.overwrite)
        ...
        pop_pca_scores_ht.write(get_ancestry_pca_ht_path('scores', args.include_unreleasable_samples), overwrite=args.overwrite)
        pop_pca_loadings_ht.write(get_ancestry_pca_ht_path('loadings', args.include_unreleasable_samples), overwrite=args.overwrite)
        with hl.utils.hadoop_open(get_ancestry_pca_ht_path('eigenvalues', args.include_unreleasable_samples), mode='w') as f:
    ...
    if args.assign_pops:
        ...
        pop_ht = pop_ht.checkpoint(pop_ht_path, overwrite=args.overwrite, _read_if_exists=not args.overwrite)
        pop_ht.transmute(
    ...
    if args.apply_stratified_filters:
        ...
        ).write(stratified_metrics_ht_path, overwrite=args.overwrite)

    if args.apply_regressed_filters:
        apply_regressed_filters(
            args.filtering_qc_metrics.split(","),
            args.include_unreleasable_samples
        ).write(regressed_metrics_ht_path, overwrite=args.overwrite)

    if args.compute_related_samples_to_drop:
        rank_ht = compute_sample_rankings(use_qc_metrics_filters=True)
        rank_ht = rank_ht.checkpoint(release_samples_rankings_ht_path, overwrite=args.overwrite, _read_if_exists=not args.overwrite)
        filtered_samples = hl.literal(rank_ht.aggregate(hl.agg.filter(rank_ht.filtered, hl.agg.collect_as_set(rank_ht.s)))) # TODO: don't localize once hail bug is fixed
        ...
        samples_to_drop.write(release_related_samples_to_drop_ht_path, overwrite=args.overwrite)

    if args.generate_metadata:
        meta_ht = generate_metadata()
        meta_ht.checkpoint(meta_ht_path, overwrite=args.overwrite, _read_if_exists=not args.overwrite)
